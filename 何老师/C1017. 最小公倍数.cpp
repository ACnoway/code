...
int searchsum(int x,int l,int r){
    ...
    pushdown(x);
    int s=0;
    if(t[x<<1].r>=l) s+=searchsum(x<<1,l,r),s%=mod;
    if(t[x<<1|1].l<=r) s+=searchsum(x<<1|1,l,r),s%=mod;
    return s;
}
...
signed main()
{
    ...
    int cnt,ans=0;
    for(int i=1;i<=n;++i){
        cnt=searchtot(1,1,a[i].id);
        addsum(1,a[i].id,ksm(2,cnt)*a[i].san%mod);
        ans=(ans+searchsum(1,a[i].id,n)*a[i].er%mod)%mod;
        cheng(1,a[i].id+1,n);
        addtot(1,a[i].id);
    }
    write(ans);
